"""MCP server cho domain dịch vụ (services).

Expose logic ``search_services`` hiện tại qua MCP.
"""
...
from service.data.data_loader_elastic_search import SERVICES_INDEX
...
async def search_services(
    ...
    if ten_dich_vu:
        ...
        query["bool"]["should"].append(
            {"match_phrase": {"ten_dich_vu": {"query": ten_dich_vu, "boost": 10.0}}}
        )

    if ten_san_pham:
        query["bool"]["should"].append(
            {"match_phrase": {"ten_san_pham": {"query": ten_san_pham, "boost": 10.0}}}
        )
        ...
        if search_terms:
            ...
            fallback_query: Dict[str, Any] = {
                "bool": {
                    ...
                    "filter": [{"term": {"customer_id": sanitized_customer_id}}],
                }
            }
            response = await es_client.search(
                index=SERVICES_INDEX,
...
async def _get_es_client() -> AsyncElasticsearch:
    """Khởi tạo/lấy lại client Elasticsearch dùng chung cho MCP server services."""
...
async def services_search(
    ...
    """Tìm kiếm dịch vụ sửa chữa cho một customer.

    Tham số:
    - customer_id: mã khách hàng (tenant).
    - thread_id: mã phiên chat.
    - query: câu hỏi gốc, tạm map sang ``ten_dich_vu`` và ``original_query``.
    - offset: phân trang.
    """
