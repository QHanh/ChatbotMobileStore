...
from chat_mcp.agents.order_agent import OrderAgent

from .common import get_messages_from_state, get_context_from_state, resolve_user_input


def make_order_agent_node(tenant_id: str, tools_for_agent: List[Any]):
    async def node(state: ChatState, config: RunnableConfig) -> ChatState:
        ...
        try:
            ...
            print(f"[ORCH-AGENT] Error in order node: {e}")
            error: Dict[str, Any] = {
                "code": "order_agent_error",
                "message": str(e),
                "detail": repr(e),
            }
            return {"error": error}
