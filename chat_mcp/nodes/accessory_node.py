...
from chat_mcp.agents.accessory_agent import AccessoryAgent

from .common import get_messages_from_state, get_context_from_state, resolve_user_input


def make_accessory_agent_node(tenant_id: str, tools_for_agent: List[Any]):
    async def node(state: ChatState, config: RunnableConfig) -> ChatState:
        ...
        try:
            agent = AccessoryAgent()
            result: AgentResult = await agent.run(agent_context)  # type: ignore[call-arg]
        except Exception as e:
            print(f"[ORCH-AGENT] Error in accessory node: {e}")
            error: Dict[str, Any] = {
                "code": "accessory_agent_error",
                "message": str(e),
                "detail": repr(e),
            }
            return {"error": error}
