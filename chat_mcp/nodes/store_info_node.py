...
from chat_mcp.agents.store_info_agent import StoreInfoAgent

from .common import get_messages_from_state, get_context_from_state, resolve_user_input


def make_store_info_agent_node(tenant_id: str, tools_for_agent: List[Any]):
    async def node(state: ChatState, config: RunnableConfig) -> ChatState:
        agent_type = "store_info"
        ...
        try:
            ...
            print(f"[ORCH-AGENT] Error in store_info node: {e}")
            error: Dict[str, Any] = {
                "code": "store_info_agent_error",
                "message": str(e),
                "detail": repr(e),
            }
            return {"error": error}
