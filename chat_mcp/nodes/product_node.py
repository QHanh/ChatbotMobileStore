...
from chat_mcp.agents.product_agent import ProductAgent

from .common import get_messages_from_state, get_context_from_state, resolve_user_input


def make_product_agent_node(tenant_id: str, tools_for_agent: List[Any]):
    async def node(state: ChatState, config: RunnableConfig) -> ChatState:
        ...
        try:
            ...
            print(f"[ORCH-AGENT] Error in product node: {e}")
            error: Dict[str, Any] = {
                "code": "product_agent_error",
                "message": str(e),
                "detail": repr(e),
            }
            return {"error": error}
