...
from chat_mcp.agents.customer_info_agent import CustomerInfoAgent

from .common import get_messages_from_state, get_context_from_state, resolve_user_input


def make_customer_info_agent_node(tenant_id: str, tools_for_agent: List[Any]):
    async def node(state: ChatState, config: RunnableConfig) -> ChatState:
        agent_type = "customer_info"
        ...
        try:
            agent = CustomerInfoAgent()
            result: AgentResult = await agent.run(agent_context)  # type: ignore[call-arg]
        except Exception as e:
            print(f"[ORCH-AGENT] Error in customer_info node: {e}")
            error: Dict[str, Any] = {
                "code": "customer_info_agent_error",
                "message": str(e),
                "detail": repr(e),
            }
            return {"error": error}
