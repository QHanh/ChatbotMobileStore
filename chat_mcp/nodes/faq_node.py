...
from chat_mcp.agents.faq_agent import FAQAgent

from .common import get_messages_from_state, get_context_from_state, resolve_user_input


def make_faq_agent_node(tenant_id: str, tools_for_agent: List[Any]):
    async def node(state: ChatState, config: RunnableConfig) -> ChatState:
        ...
        try:
            ...
            print(f"[ORCH-AGENT] Error in faq node: {e}")
            error: Dict[str, Any] = {
                "code": "faq_agent_error",
                "message": str(e),
                "detail": repr(e),
            }
            return {"error": error}
